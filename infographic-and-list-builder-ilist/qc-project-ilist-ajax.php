		...
		<div id="ilist-modal" class="ilistmodaltemplate">
			...
			<div class="modal-content-template" data="<?php echo esc_attr($list_type); ?>">
				...
				<?php foreach($templatearray as $key=>$val) : ?>
					...
					<?php 
						if($key != 'simple'){
							echo '<h2 style="margin: 2em 0;"><span style="color:red">'.esc_html( 'Pro Templates (Need Pro version)', 'iList' ).'</span></h2><hr />';
						}
					?>
...
function render_shortcode_modal() {
	...
	<div id="ilist-modal" class="ilist_shortcode_modal">
		...
		<div class="ilist-modal-content">
			<span class="close">×</span>
			<h3><img src="<?php echo plugins_url( 'assets/images/1.png', __FILE__ ) ?>" alt="<?php echo esc_attr( 'iList - Free' , 'iList' ); ?>"> <?php esc_html_e( ' - Shortcode Maker' , 'iList' ); ?></h3>
			<hr/>
			<div class="sm_shortcode_list">
				...
				<div class="ilist_single_field_shortcode">
					<label style="width: 200px;display: inline-block;"> <?php echo esc_html( 'Order By' , 'iList' ); ?> </label><select style="width: 225px;" id="ilist_item_orderby">
...
function qcld_openai_title_generate_desc() {
    ...
    if ( !current_user_can('manage_options') ) {
	    ...
	    echo wp_send_json( $qcld_ilist_result );
	    wp_die();
    ...
    if(!empty($title)){
        ...
        $gptkeyword = [];
        if($ai_engines == 'gpt-3.5-turbo' || $ai_engines == 'gpt-4' || $ai_engines == 'gpt-4o'){
            $ch = curl_init();
            ...
            $result_data 	= isset( $complete->choices[0]->message->content ) ? trim( $complete->choices[0]->message->content ) : '';

            /* string #### to replace h3 tag */
            $result_data = preg_replace( '/^####\s+(.*)$/m',"$1", $result_data);
		...
		//If li-id not exists in the cookie, then prceed to vote
			...
			if(!empty($results)){
				...
				foreach( $results as $ke => $value ){
					...
					foreach( $qcld_response as $title_value ){
				        ...
				        if($ai_engines == 'gpt-3.5-turbo' || $ai_engines == 'gpt-4' || $ai_engines == 'gpt-4o'){
					        ...
					        $meta_desc_prompt = "rewrite this paragraph for a unique artical:\n\n " . $title_value.'. Max: 300 characters.';
					        ...
					        $result_short_data 	= isset( $complete->choices[0]->message->content ) ? trim( $complete->choices[0]->message->content ) : '';

				            /* string #### to replace h3 tag */
				            $result_short_data = preg_replace( '/^####\s+(.*)$/m',"$1", $result_short_data );
				...
				$qcld_ilist_result['status']  = 'success';
            	$qcld_ilist_result['msg']     = 'Data Successfully Submitted.';	
