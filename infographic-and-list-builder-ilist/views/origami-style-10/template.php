...
<div id="qcld-list-holder" class="qcld-list-hoder pinfog5bgimage" <?php echo $hoderbg; ?>>
<div id="qcopd-list-<?php echo $listId; ?>" >
	...
	<div style="clear:both;margin-bottom:20px"></div>
		<?php
	if(isset($ilist_chart[0]) && !empty($ilist_chart[0]) and $show_chart_position[0]=='top'){
	?>
	...
	<?php foreach($list_sl as $list) : ?>
		<li id="qcld_sl_<?php echo get_the_ID()."_".$cnt ?>" class="list-style-seventeen list-style-seventeen-01 ilist_pinfo5_<?php echo $column ?>">
			<div class="list-seventeen-inner-part pinfo5hoveritem ori10itembgcolor" style="<?php echo ( ( isset($list['qcld_entrybg_color']) && !empty($list['qcld_entrybg_color']) ) ? 'background:'.$list['qcld_entrybg_color']:''); ?>">
				<div class="list-seventeen-sl">
					...
					<?php }elseif(isset($list['qcld_text_image_fa']) && !empty($list['qcld_text_image_fa'])){
							...
							echo '<div class="list-seventeen-avatar"><div style="font-size: 54px;color: #fff;align-self: center;"><i class="fa fa-check"></i></div></div>';
						} 
						?>
				</div>
				...
				<div class="upvote-section" style="display: inline-block;margin-left: 10px;cursor: pointer;">
					<span data-post-id="<?php echo get_the_ID(); ?>" data-item-title="<?php echo isset($list['qcld_text_title']) ? trim($list['qcld_text_title']):''; ?>" class="upvote-btn-ilist upvote-on" style="line-height: 48px;">
		...
		<?php
	if(isset($ilist_chart[0]) && !empty($ilist_chart[0]) and $show_chart_position[0]=='bottom'){
	?>
	<div class="ilist_chart_map_wrap">
		<?php echo do_shortcode($ilist_chart[0]);?>
	</div>
